...
export const notificationService = {
  ...
  getNotifications: async (params?: { page?: number; size?: number; unreadOnly?: boolean }): Promise<{ data: PageResponse<Notification> }> => {
    ...
    return {
      data: {
        ...data,
        content: data.content.map((n) => toNotification(n, n.senderId != null ? (userMap.get(String(n.senderId)) ?? null) : null)),
      },
    };
  },

  /** 미확인 알림 개수 — GET /api/notifications/unread-count */
  getUnreadCount: () => api.get<Record<string, number>>("/notifications/unread-count").then((res) => ({ ...res, data: res.data ?? 0 })) as Promise<{ data: number }>,

  /** 알림 읽음 처리 — PATCH /api/notifications/{id}/read  (id='all' → 전체) */
  markRead: (id: string | "all") => api.patch<{ unreadCount: number }>(`/notifications/${id}/read`),
  ...
  getSettings: () =>
    api.get<NotificationSettings>("/notifications/settings").then((r) => ({ data: r.data })),

  /** 알림 설정 변경 — PATCH /api/notifications/settings */
  updateSettings: (settings: Partial<NotificationSettings>) =>
    api.patch<NotificationSettings>("/notifications/settings", settings).then((r) => ({ data: r.data })),

  // ── Device tokens (FCM push) ──────────────────────────────────────────────

  /** 디바이스 토큰 등록 — POST /api/users/me/device-tokens */
  registerDeviceToken: (token: string, platform: "android" | "ios" | "web") => api.post("/users/me/device-tokens", { token, platform }),

  /** 디바이스 토큰 해제 — DELETE /api/users/me/device-tokens */
  removeDeviceToken: (token: string) => api.delete("/users/me/device-tokens", { data: { token } }),
};
