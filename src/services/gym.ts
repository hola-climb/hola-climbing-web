...
import type {
  FeedVideo, Gym, GymDetail, GymGrade, GymPhoto, GymReview,
  OperatingHours, PageResponse, RawRecommendedVideo, Video,
} from '@/types/api'
...
interface RawGymPhoto { id: number; gcsPath: string; caption: string | null; displayOrder: number }
...
interface RawDayHours { open: string | null; close: string | null }
...
function toOperatingHours(bh: Record<string, RawDayHours | null> | null): OperatingHours | null {
  ...
  for (const d of days) {
    const v = norm[d]
    if (v && v.open && v.close) { out[d] = `${v.open} - ${v.close}`; any = true }
    else out[d] = null
  }
  return any ? out : null
}
...
export const gymService = {
  ...
  search: (params: {
    ...
    page?: number
    size?: number
  }) => api.get<PageResponse<RawGymSummary>>('/gyms', { params })
    .then((res) => ({
      ...res,
  ...
  getNearby: (params: { lat: number; lng: number; radius?: number; size?: number }) =>
    api.get<RawGymSummary[]>('/gyms/nearby', { params })
      .then((res) => ({ ...res, data: res.data.map(toGym) })) as Promise<{ data: Gym[] }>,

  getGym: (id: string) =>
    api.get<RawGymDetail>(`/gyms/${id}`)
      .then((res) => ({ ...res, data: toGymDetail(res.data) })) as Promise<{ data: GymDetail }>,

  getGymVideos: (gymId: string, params?: { page?: number; size?: number; sort?: string }) =>
    api.get<PageResponse<Omit<RawRecommendedVideo, 'source'>>>(`/gyms/${gymId}/videos`, { params })
      .then((res) => ({
        ...res,
  ...
  getGrades: (gymId: string) =>
    api.get<GymGrade[]>(`/gyms/${gymId}/grades`),

  // ── Photos ─────────────────────────────────────────────────────────────────

  getPhotos: (gymId: string) =>
    api.get<RawGymPhoto[]>(`/gyms/${gymId}/photos`)
      .then((res) => ({ ...res, data: res.data.map(toGymPhoto) })) as Promise<{ data: GymPhoto[] }>,
  ...
  createReview: async (gymId: string, rating: number, content: string): Promise<{ data: GymReview }> => {
    ...
    return {
      data: {
        id: String(r.id), gymId: String(r.gymId), user, rating: r.rating,
  ...
  deleteReview: (reviewId: string) =>
    api.delete(`/gyms/reviews/${reviewId}`),

  // ── Favorites ─────────────────────────────────────────────────────────────

  addFavorite: (gymId: string) =>
    api.post<{ isFavorited: boolean }>(`/favorites/gyms/${gymId}`),

  removeFavorite: (gymId: string) =>
    api.delete<{ isFavorited: boolean }>(`/favorites/gyms/${gymId}`),

  getFavorites: (params?: { page?: number; size?: number }) =>
    api.get<PageResponse<RawGymSummary>>('/favorites/gyms', { params })
      .then((res) => ({
        ...res,
        data: { ...res.data, content: res.data.content.map(toGym) },
      })) as Promise<{ data: PageResponse<Gym> }>,
  ...
  getBoard: (gymId: string, params?: { page?: number; size?: number }) =>
    api.get<PageResponse<BoardPost>>(`/gyms/${gymId}/board`, { params }),

  createBoardPost: (gymId: string, content: string, currentLat?: number, currentLng?: number) =>
    api.post<BoardPost>(`/gyms/${gymId}/board`, { content, currentLat, currentLng }),

  deleteBoardPost: (boardId: string) =>
    api.delete(`/gyms/board/${boardId}`),

  // ── Recommendations ───────────────────────────────────────────────────────

  getRecommended: (limit = 10) =>
    api.get<RawGymSummary[]>('/recommendations/gyms', { params: { limit } })
      .then((res) => ({ ...res, data: res.data.map(toGym) })) as Promise<{ data: Gym[] }>,
}
